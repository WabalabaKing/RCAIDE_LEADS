# -*- coding: utf-8 -*-
"""
Created on Sat Aug 23 16:37:19 2025
...
# local imports 
Currdir=os.getcwd()
sys.path.append(os.path.join( os.path.split(os.path.split(Currdir)[0])[0], 'Vehicles'))
...
# ---------------------------------------------------------------------- 
def main(): 
    VLMresults,SU2results = Boeing_737_Drag_Polar()
    #------------------------------------------------------------------------
    # setup figures
    #------------------------------------------------------------------------
    fig = plt.figure()  
    ...
    axis_1.plot( VLMresults.alpha/Units.degree, VLMresults.lift_coefficient, 'bo-',  label = 'VLM') 
    axis_1.plot( VLMresults.alpha/Units.degree, SU2results.lift_coefficient, 'rs-',  label = 'SU2') 
    axis_2.plot( VLMresults.alpha/Units.degree, VLMresults.drag_coefficient, 'bo-',  label = 'VLM') 
    axis_2.plot( VLMresults.alpha/Units.degree, SU2results.drag_coefficient, 'rs-',  label = 'SU2') 
...
def Boeing_737_Drag_Polar():
    ...
    aerodynamics_analysis_routineVLM         = RCAIDE.Framework.Analyses.Aerodynamics.Vortex_Lattice_Method()
    aerodynamics_analysis_routineSU2         = RCAIDE.Framework.Analyses.Aerodynamics.SU2_Euler()
    aerodynamics_analysis_routineVLM.vehicle = vehicle
    aerodynamics_analysis_routineSU2.vehicle = vehicle
    ...
    VLMresults                           = aircraft_aerodynamic_analysis(aerodynamics_analysis = aerodynamics_analysis_routineVLM,
                                                                      angle_of_attacks = angle_of_attack_range,
                                                                      mach_numbers = Mach_number_range,
                                                                      temperatures                     = temperatures,
                                                                      altitude  = 0)
    SU2results                           = aircraft_aerodynamic_analysis(aerodynamics_analysis = aerodynamics_analysis_routineSU2,
                                                                      angle_of_attacks = angle_of_attack_range,
                                                                      mach_numbers = Mach_number_range,
                                                                      temperatures                     = temperatures,
                                                                      altitude  = 0)
